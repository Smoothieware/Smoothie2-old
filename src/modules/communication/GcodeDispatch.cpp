                    ...
                    if(gcode->has_m) {
                        switch (gcode->m) {
                            ...
                            {    // concatenate the command again and send to panel if enabled
                                ...
                                // TOADDBACK : PublicData::set_value( panel_checksum, panel_display_message_checksum, &str );
                                delete gcode;
                                new_message.stream->printf("ok\r\n");
                                return;
                            }
                            ...
                            case 500: // M500 save volatile settings to config-override
                                // TOADDBACK : THEKERNEL->conveyor->wait_for_empty_queue(); //just to be safe as it can take a while to run
