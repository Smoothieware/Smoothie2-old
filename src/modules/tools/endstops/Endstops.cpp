    ...
    if (this->is_delta || this->is_scara) {
        ...
        for ( int c = X_AXIS; c <= Z_AXIS; c++ ) {
            if ( this->trim_mm[c] != 0.0F && ( axes_to_move >> c ) & 1 ) {
                ...
                this->feed_rate[c]= this->slow_rates[c];
                // TOADDBACK : used lrint instead of round here to statisfy compiler, but no idea what I'm doing
                STEPPER[c]->move(inverted_dir, abs(lrint(this->trim_mm[c]*STEPS_PER_MM(c))), 0);
            }
        }
