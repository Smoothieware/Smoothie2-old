...
void Switch::on_gcode_received(void *argument)
{
    ...
    gcode->stream->printf("SlowTicker, interval:%f, max_frequency:%f, hooks:%d \n", THEKERNEL->slow_ticker->interval, THEKERNEL->slow_ticker->max_frequency, THEKERNEL->slow_ticker->hooks.size() );
