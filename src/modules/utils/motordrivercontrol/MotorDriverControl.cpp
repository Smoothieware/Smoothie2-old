...
#define hold_current_checksum          CHECKSUM("hold_current") //TMC2130 has a hold current ration of 1/32 to 32/32
...
bool MotorDriverControl::config_module(uint16_t cs)
{
    ...
    if(str == "DRV8711") {
        ...
        chip= TMC2130;
        tmc21x= new TMC21X(std::bind( &MotorDriverControl::sendSPI, this, _1, _2, _3), designator);
    }else if(str == "TMC2660") {
        chip= TMC2660;
    ...
    // set default max currents for each chip, can be overridden in config
    switch(chip) {
        case DRV8711: max_current= DRV8711_max_current; break;
        case TMC2660: max_current= TMC2660_max_current; break;
        case TMC2130: max_current= TMC2130_max_current; break;  //TMC2130 only allows a ratio of 0..31 which represents 1/32 .. 32/32
    ...
    max_current= THEKERNEL->config->value(motor_driver_control_checksum, cs, max_current_checksum )->by_default((int)max_current)->as_number();
    //current_factor= THEKERNEL->config->value(motor_driver_control_checksum, cs, current_factor_checksum )->by_default(1.0F)->as_number();

    current= 		THEKERNEL->config->value(motor_driver_control_checksum, cs, current_checksum      )->by_default((int)max_current)->as_number(); // set to maxCurrent as TMC2130 uses a ratio method and not mA
    hold_current= 	THEKERNEL->config->value(motor_driver_control_checksum, cs, hold_current_checksum )->by_default((int)max_current)->as_number(); // set to maxCurrent as TMC2130 uses a ratio method and not mA
...
void MotorDriverControl::set_options(Gcode *gcode)
{
    switch(chip) {
        ...
        case TMC2130: {
            ...
            // options.clear();
            // if(tmc21x->get_optional(options)) {
            //     // foreach optional value
            //     for(auto &i : options) {
            //         // print all current values of supported options
            //         gcode->stream->printf("%c: %d ", i.first, i.second);
